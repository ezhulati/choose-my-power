...
export const AccessibilityAuditor: React.FC<AccessibilityAuditorProps> = ({
  ...
  const [auditReport, setAuditReport] = useState<unknown>(null);
  const [colorTests, setColorTests] = useState<unknown>(null);
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          disabled={isAuditing}
          className="px-4 py-2 bg-texas-navy text-white rounded-lg hover:bg-texas-navy-800 disabled:opacity-50 transition-colors focus:ring-4 focus:ring-blue-200 focus:outline-none"
        >
      ...
      {auditReport && (
        <div className="mb-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div className="text-center p-3 bg-texas-red/10 rounded-lg">
              <div className="text-2xl font-bold text-texas-red">{auditReport.summary.errors}</div>
              <div className="text-xs text-texas-red">Errors</div>
            </div>
            <div className="text-center p-3 bg-yellow-50 rounded-lg">
              <div className="text-2xl font-bold text-yellow-600">{auditReport.summary.warnings}</div>
              <div className="text-xs text-yellow-600">Warnings</div>
            </div>
            <div className="text-center p-3 bg-texas-navy/10 rounded-lg">
              <div className="text-2xl font-bold text-texas-navy">{auditReport.summary.infos}</div>
              <div className="text-xs text-texas-navy">Info</div>
            </div>
          </div>
        </div>
      )}
            ...
            {Object.entries(colorTests).map(([key, colors]: [string, any]) => (
              <div key={key} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className={`px-2 py-1 rounded text-xs ${colors.ratio >= 4.5 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-texas-red'}`}>
            ...
            {violations.map((violation, index) => (
              <div 
                ...
                className={`p-3 rounded-lg border-l-4 ${
                  violation.severity === 'error' ? 'border-red-500 bg-texas-red/10' :
