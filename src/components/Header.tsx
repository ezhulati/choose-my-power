        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center">
            <Button
              ...
              size="lg"
              onClick={() => onNavigate('/')}
              className="flex items-center space-x-2 text-texas-navy hover:text-texas-navy hover:bg-texas-cream-100 p-2 h-auto"
            >
            ...
            {navigation.map((item) => (
              item.dropdown ? (
                  ...
                  <DropdownMenuContent className="w-56" align="start">
                    {item.dropdown.map((dropdownItem) => (
                      <DropdownMenuItem
                        ...
                        className="cursor-pointer hover:bg-texas-cream-100 hover:text-texas-navy transition-colors"
                      >
                        {dropdownItem.name}
                      </DropdownMenuItem>
                    ))}
                ...
                <Button
                  ...
                  className="text-gray-700 hover:text-texas-navy px-4 py-2 text-sm font-medium transition-colors rounded-md hover:bg-texas-cream-200"
                >
                  {item.name}
                </Button>
              )
          ...
          <div className="md:hidden">
            <Button
              ...
              className="text-gray-700 hover:text-texas-navy hover:bg-texas-cream-100"
              ...
              {navigation.map((item) => (
                <div key={item.name}>
                  <Button
                    ...
                    className="flex items-center justify-between w-full text-left px-3 py-2 text-base font-medium text-gray-700 hover:text-texas-navy hover:bg-texas-cream-200 rounded-md transition-colors h-auto"
                  >
                    <span>{item.name}</span>
                    {item.dropdown && (
                      <ChevronDown 
                    ...
                    <div className="ml-4 space-y-1 mt-1">
                      {item.dropdown.map((dropdownItem) => (
                        <Button
                          ...
                          variant="ghost"
                          onClick={() => handleDropdownClick(dropdownItem.href)}
                          className="w-full text-left px-3 py-2 text-sm text-gray-600 hover:text-texas-navy hover:bg-texas-cream-200 rounded-md transition-colors h-auto justify-start"
                        >
                          {dropdownItem.name}
                        </Button>
                      ))}
