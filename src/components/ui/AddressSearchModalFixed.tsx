...
export const AddressSearchModal: React.FC<AddressSearchModalProps> = ({
  ...
  const handleProceedToOrder = () => {
    if (selectedLocation) {
      ...
      const planObjectId = planData.apiPlanId || '68b84e0e206770f7c563793b';
      const orderUrl = `https://orders.comparepower.com/order/service_location?esiid=${selectedLocation.esiid}&plan_id=${planObjectId}&usage=1000&zip_code=${zipCode}&utm_source=choose-my-power`;
      ...
      <div className="space-y-3 max-h-60 overflow-y-auto">
        {searchResults.map((location, index) => (
          <Card 
            key={location.esiid}
            className={cn(
              "cursor-pointer border-2 transition-all duration-200 hover:border-texas-navy focus-within:border-texas-navy focus-within:ring-2 focus-within:ring-texas-navy/20",
